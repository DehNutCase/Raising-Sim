...
@export var param_mouth_name: String = "ParamO"
...
func _on_cubism_process(model: GDCubismUserModel, delta: float):
	...
	#handles what happens when the sound stopped playing
	if history_position >= ary_volume_history.size():
		history_position = 0
		#Added so that we don't keep lip synching when no voice is played
	...
	if param_mouth != null:
		...
		var vol = AudioServer.get_bus_peak_volume_left_db(bus_index, 0)
		if avg_v > 0.1 and vol > -200:
